...
pub async fn handle_subscription_command(
    ...
    match matches.subcommand() {
        Some(("subscribe", sub_m)) => {
            let user_id = *sub_m.get_one::<i32>("user").ok_or_else(|| anyhow::anyhow!("Missing required argument: user"))?;
            let plan_id = *sub_m.get_one::<i32>("plan").ok_or_else(|| anyhow::anyhow!("Missing required argument: plan"))?;
            let duration = *sub_m.get_one::<i64>("duration").ok_or_else(|| anyhow::anyhow!("Missing required argument: duration"))?;
