    ...
    match matches.subcommand() {
        Some(("create", sub_m)) => {
            ...
            let monthly_fee = *sub_m.get_one::<i64>("monthly-fee")
                .ok_or_else(|| anyhow!("Missing required argument: --monthly-fee"))?;
            let billing_mode_str = sub_m.get_one::<String>("billing-mode")
                .ok_or_else(|| anyhow!("Missing required argument: --billing-mode"))?;
            ...
            let channel_id = *sub_m.get_one::<i32>("channel-id")
                .ok_or_else(|| anyhow!("Missing required argument: --channel-id"))?;
