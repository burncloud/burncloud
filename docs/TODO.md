# [ ] 通过提交的头或者特定参数来决定往哪个平台转发，因为某此情况下面有一些新的路由链接会生成，我们在这种情况下面是不会及早的发现，所以只能通过参数来提早判断。而不应该通过路由的链接来决定。

# [ ] 系统应该有查询amazon是否有cloudwatch再或者 cloudtrail，如果有，则需要定期分析这个账号的安全度
# [ ] 每个版本应该对程序进行版本比对，确认程序没有被女巫修改过，应该需要质押，或者持续检测。消费记录，等账号比对。
...
# [ ] 泛型透传与协议降级 (Generic Passthrough & Protocol Degradation)
