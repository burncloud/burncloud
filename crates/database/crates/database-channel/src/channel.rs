...
impl ChannelModel {
    ...
    pub async fn delete(db: &Database, id: i32) -> Result<()> {
        ...
        let sql_abilities = adapt_sql(is_postgres, "DELETE FROM abilities WHERE channel_id = ?");
        sqlx::query(&sql_abilities).bind(id).execute(pool).await?;

        // Delete Channel
        let sql_channels = adapt_sql(is_postgres, "DELETE FROM channels WHERE id = ?");
    ...
    pub async fn sync_abilities(db: &Database, channel: &Channel) -> Result<()> {
        ...
        let sql_delete = adapt_sql(is_postgres, "DELETE FROM abilities WHERE channel_id = ?");
