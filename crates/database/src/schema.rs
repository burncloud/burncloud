...
impl Schema {
    pub async fn init(db: &Database) -> Result<()> {
        ...
        // Prices are synced from remote pricing_data repository via PriceSyncService
        // See: crates/router/src/price_sync.rs
        // No hardcoded fallback prices - system requires remote sync on first run
