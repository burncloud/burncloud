...
fn fake_node(pid: u32) -> FakeNode {
    NodeComposition::start(
        FakeHardwareProbe::default(),
        FakeArtifactPreparer,
        FakeRuntimePreparer,
        FakeProcessManager::new(pid),
        FakeReadinessProbe,
        FakeHealthProbe,
    )
}

/// Phase-0.5 acceptance: every success evidence item follows a successful call
/// to the owner capability. Supply resolution stays outside platform composition.
#[tokio::test]
async fn fake_golden_path_converges_from_absent_to_routable() {
    ...
    assert!(!node.hardware().inspect().await.unwrap().accelerators.is_empty());
...
async fn fake_golden_path_cannot_serve_before_router_attachment_evidence() {
    ...
    let resolved = resolver.resolve(ModelResolutionRequest { model: "qwen-4b".into() }).await.unwrap();
    node.reconciler_mut().observe(ReconcileEvidence::Resolved).unwrap();

    let artifact = node.artifacts().prepare(ArtifactRequest { source: resolved.artifact, expected_digest: None }).await.unwrap();
    node.reconciler_mut().observe(ReconcileEvidence::ArtifactPrepared).unwrap();
    node.reconciler_mut().next_action().unwrap();
    let runtime = node.runtimes().prepare(RuntimeRequest { runtime: resolved.runtime, version: None }).await.unwrap();
    node.reconciler_mut().observe(ReconcileEvidence::RuntimePrepared).unwrap();
