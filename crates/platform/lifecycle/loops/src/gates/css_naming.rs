...
pub fn run_css_naming(root: &Path) -> anyhow::Result<(bool, Vec<String>)> {
    ...
    let mut lines = vec![
        "CSS optimize acceptance (see crates/platform/lifecycle/loops/acceptance/css-optimize-acceptance.md)".to_string(),
        String::new(),
    ];
    ...
    if violations > 0 {
        ...
        lines.push(format!(
            "FAIL: {violations} issue(s). Fix per crates/platform/lifecycle/loops/acceptance/css-optimize-acceptance.md"
        ));
        Ok((false, lines))
    } else {
        lines.push(String::new());
