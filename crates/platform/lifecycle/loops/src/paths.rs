...
/// Repository root (`burncloud/`), inferred from the lifecycle loops crate.
pub fn repo_root() -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .and_then(|p| p.parent())
        .and_then(|p| p.parent())
        .expect("loops must live at crates/platform/lifecycle/loops")
        .to_path_buf()
}
...
pub fn client_crate_dir(root: &Path) -> PathBuf {
    root.join("crates").join("interfaces").join("client")
}
