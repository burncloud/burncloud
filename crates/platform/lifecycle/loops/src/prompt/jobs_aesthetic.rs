...
pub fn build_jobs_aesthetic_prompt(input: &PromptInput) -> anyhow::Result<PathBuf> {
    ...
    lines.push("- Run: cd crates/interfaces/client; cargo check -p burncloud-client-shared".to_string());
    lines.push(format!(
        "- Update review.json entry for `{}` only; leave other pages unchanged",
        input.focus_page
    ));
    lines.push("- Constraints: crates/platform/lifecycle/loops/acceptance/jobs-aesthetic-agent-prompt.md".to_string());
