...
pub fn run(opts: CssOptimizeOptions) -> anyhow::Result<i32> {
    ...
    if !acceptance.exists() || !agent_rules.exists() {
        anyhow::bail!("missing acceptance docs under crates/platform/lifecycle/loops/acceptance/");
    }
    ...
    let naming_prompt = format!(
        "Read crates/platform/lifecycle/loops/acceptance/css-optimize-acceptance.md and css-optimize-agent-prompt.md. \
         Fix STATIC naming violations (gate css-naming). One iteration, max 8 files. Work in {}.",
        root.display()
    );
    let visual_prompt = format!(
        "Read crates/platform/lifecycle/loops/acceptance/css-optimize-acceptance.md section V and css-optimize-agent-prompt.md section 3. \
