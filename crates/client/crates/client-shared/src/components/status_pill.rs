...
pub fn StatusPill(value: String, label: Option<String>) -> Element {
    let (class, display) = match value.to_lowercase().as_str() {
        "active" | "running" | "ok" | "success" => {
            ("success", label.unwrap_or_else(|| value.clone()))
        }
        "throttle" | "rate_limited" | "warning" => {
            ("warning", label.unwrap_or_else(|| value.clone()))
        }
        "down" | "error" | "failed" | "danger" => {
            ("danger", label.unwrap_or_else(|| value.clone()))
        }
