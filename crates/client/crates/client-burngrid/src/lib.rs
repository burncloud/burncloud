        ...
        div { class: "page-content",
            ...
            div { class: "grid mb-12",
                ...
                div { class: "flex flex-col p-4",
                    span { class: "text-xs font-bold text-gray-400 uppercase tracking-widest mb-3", "Active Nodes" }
                ...
                div { class: "flex flex-col p-4 border-l border-base-200/50", // Subtle divider instead of box
                    span { class: "text-xs font-bold text-gray-400 uppercase tracking-widest mb-3", "Total Requests" }
                ...
                div { class: "flex flex-col p-4 border-l border-base-200/50",
                    span { class: "text-xs font-bold text-gray-400 uppercase tracking-widest mb-3", "Estimated Revenue" }
                    span { class: "text-4xl font-bold text-base-content tracking-tight",
            ...
            div { class: "flex justify-between items-end pb-4 border-b border-base-200 mb-8",
                h2 { class: "text-lg font-bold text-base-content m-0", "Resource Pool" }
                ...
                div { class: "card border border-dashed border-base-300 bg-base-100",
                    div { class: "p-16 text-center",
                        div { class: "flex flex-col items-center gap-6",
                            ...
                            div {
                                h3 { class: "text-xl font-bold text-base-content mb-2", "Empty Resource Pool" }
                                p { class: "text-sm text-gray-500 max-w-md mx-auto leading-relaxed",
                                    "Click 'Add Resource' above to fill your asset matrix,"
                                    br {}
                                    "and start building your automated profit engine."
                                }
                            }
                            button {
...
fn DeployDialog(
    ...
    let on_start = move |_| {
        ...
        let f_path = match selected_file() {
            Some(f) => f,
            None => {
                error_msg.set(Some("Please select a GGUF file".to_string()));
                return;
            }
        };
        // ... logic ...
        ...
        spawn(async move {
            deploy_status.set(Some("Starting service...".to_string()));
            ...
            match burncloud_service_inference::InferenceService::new().await {
                Ok(service) => match service.start_instance(config).await {
                    Ok(_) => {
                        deploy_status.set(Some("Service started successfully!".to_string()));
                        on_deploy_success.call(());
                    }
                    Err(e) => {
                        error_msg.set(Some(format!("Start failed: {}", e)));
        ...
        div {
            ...
            div {
                ...
                div { class: "p-lg flex justify-between items-center border-b",
                    h2 { class: "text-title font-semibold m-0", "Deploy Model: {model_id}" }
                    button { class: "btn btn-secondary", onclick: move |_| on_close.call(()), "✕" }
                }

                div { class: "p-lg flex flex-col gap-md",
                    if loading() {
                        div { class: "text-center", "Loading files..." }
                    } else if files.read().is_empty() {
                         div { class: "text-danger", "No GGUF files found. Please download model files first." }
                    } else {
                        // File Selection
                        div {
                            label { class: "block text-sm font-medium mb-xs", "Select File (GGUF)" }
                            select {
                                class: "input w-full",
                        ...
                        // Port
                        div {
                            label { class: "block text-sm font-medium mb-xs", "Port" }
                            input {
                                class: "input w-full",
                                r#type: "number",
                                value: "{port}",
                        ...
                        // Context Size
                        div {
                            label { class: "block text-sm font-medium mb-xs", "Context Size" }
                            select {
                                class: "input w-full",
                        ...
                        // GPU Layers
                        div {
                            label { class: "block text-sm font-medium mb-xs", "GPU Layers (-1 = All, 0 = CPU only)" }
                            input {
                                class: "input w-full",
                                r#type: "number",
                                value: "{gpu_layers}",
        ...
        div {
            ...
            div {
                ...
                div { class: "p-lg border-b",
                    h2 { class: "text-title font-semibold m-0", "Confirm Delete?" }
                }
    ...
    use_effect(move || {
        ...
        spawn(async move {
            match burncloud_service_models::get_model_files(&id).await {
                ...
                Err(e) => {
                    error_msg.set(Some(format!("Failed to get file list: {}", e)));
                    loading.set(false);
                }
            }
        });
        ...
        div {
            ...
            div {
                ...
                div { class: "p-lg flex justify-between items-center",
                    ...
                    h2 { class: "text-title font-semibold m-0", "Model Files: {model_id}" }
                    button { class: "btn btn-secondary", onclick: move |_| on_close.call(()), "✕" }
                }
                ...
                div { class: "p-lg", style: "flex: 1; overflow-y: auto;",
                    ...
                    if loading() {
                        div { class: "text-center p-xxxl", "Loading..." }
                                                            ...
                                                            rsx! {
                                                                button {
                                                                    ...
                                                                    onclick: move |_| {
                                                                        ...
                                                                        spawn(async move {
                                                                            download_status.set(Some(format!("Starting download {}...", f_path)));
                                                                            match burncloud_service_models::download_model_file(&m_id, &f_path).await {
                                                                                Ok(_) => download_status.set(Some(format!("Added to download queue: {}", f_path))),
                                                                                Err(e) => download_status.set(Some(format!("Download failed: {}", e))),
                                                                            }
                                                                        });
                                                                    },
                                                                    "Download"
    ...
    rsx! {
        div {
            ...
            div {
                ...
                div { class: "p-lg flex justify-between items-center",
                    style: "border-bottom: 1px solid var(--color-border);",
                    h2 { class: "text-title font-semibold m-0", "Add Model" }
                    button {
                        class: "btn btn-secondary",
                        onclick: move |_| on_close.call(()),
                        "✕"
                ...
                div { class: "flex-1",
                    ...
                    div { class: "flex gap-sm items-center",
                        ...
                        if model.private.unwrap_or(false) {
                            BCBadge { variant: BadgeVariant::Warning, "🔒 Private" }
                        }
                    }
...
// Import HuggingFace API model to local database
