...
pub async fn buyer_playground_chat(
    token_ref: &str,
    model: &str,
    prompt: &str,
) -> Result<String, String> {
    let response = with_auth(Client::new().post(url("/console/api/playground/chat")))
    ...
    if !status.is_success() {
        return Err(value
            ...
            .unwrap_or("Playground request failed")
            .to_string());
    }
    value
        ...
        .map(str::to_string)
        .ok_or_else(|| "Playground response did not include assistant content".to_string())
}
