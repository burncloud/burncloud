...
fn health_tone(score: u8) -> &'static str {
    if score >= 95 {
        "tone-green"
    } else if score >= 80 {
        "tone-amber"
...
fn event_tone(event_type: &str) -> &'static str {
    if event_type == "server_error" { "badge-error" } else { "badge-warning" }
}
...
pub fn Guardrails() -> Element {
    ...
    let summary_error = summary_snapshot.as_ref().and_then(|result| result.as_ref().err().cloned());
    let summary: Option<SecuritySummary> = summary_snapshot.clone().and_then(Result::ok);

    let filters_loading = filter_snapshot.is_none();
    let filters_error = filter_snapshot.as_ref().and_then(|result| result.as_ref().err().cloned());
    let filters_ready = filter_snapshot.as_ref().is_some_and(Result::is_ok) && synced();

    let events_loading = event_snapshot.is_none();
    let events_error = event_snapshot.as_ref().and_then(|result| result.as_ref().err().cloned());
    let events: Option<RiskEventPage> = event_snapshot.clone().and_then(Result::ok);

    let breaker_connected = breaker_snapshot.as_ref().is_some_and(Result::is_ok);
    let breaker_loading = breaker_snapshot.is_none();
    let breaker_error = breaker_snapshot.as_ref().and_then(|result| result.as_ref().err().cloned());
    ...
    let enabled_controls = if filters_ready {
        usize::from(content_enabled) + usize::from(blacklist_enabled) + rules.len()
    } else {
        0
    };

    let health_value = summary.as_ref().map(|value| format!("{}/100", value.score)).unwrap_or_else(|| "—".to_string());
    let health_note = if summary_loading {
        "loading recent request health".to_string()
    } else if summary_error.is_some() {
        "recent request health unavailable".to_string()
    } else {
        "derived from recent HTTP success/error ratio".to_string()
    };
    let health_icon_tone = summary.as_ref().map(|value| health_tone(value.score)).unwrap_or("tone-gray");
    let error_event_value = summary.as_ref().map(|value| value.blocked_count.to_string()).unwrap_or_else(|| "—".to_string());
    let affected_value = summary.as_ref().map(|value| value.threat_source_count.to_string()).unwrap_or_else(|| "—".to_string());
    let policy_value = if filters_ready { enabled_controls.to_string() } else { "—".to_string() };
    ...
    rsx! {
        div { class: "page",
            ...
            if summary_error.is_some() || filters_error.is_some() || events_error.is_some() || breaker_error.is_some() {
                div { class: "card card-pad stack",
                    strong { class: "danger", "Some protection data is unavailable" }
                    if let Some(message) = summary_error.clone() { code { class: "terminal", "Request health summary: {message}" } }
                    if let Some(message) = filters_error.clone() { code { class: "terminal", "Protection policy: {message}" } }
                    if let Some(message) = events_error.clone() { code { class: "terminal", "HTTP risk signals: {message}" } }
                    if let Some(message) = breaker_error.clone() { code { class: "terminal", "Circuit breaker: {message}" } }
                }
            }

            div { class: "product-note",
                strong { "How to read this page: " }
            ...
            div { class: "metrics",
                div { class: "card metric",
                    div { class: "metric-copy", span { class: "metric-label", "Request Health" } span { class: "metric-value", "{health_value}" } span { class: "metric-note", "{health_note}" } }
                    div { class: "metric-icon {health_icon_tone}", Icon { name: "activity" } }
                }
                div { class: "card metric",
                    div { class: "metric-copy", span { class: "metric-label", "HTTP Error Events" } span { class: "metric-value", "{error_event_value}" } span { class: "metric-note", "4xx + 5xx in recent log sample" } }
                    div { class: "metric-icon tone-amber", Icon { name: "logs" } }
                }
                div { class: "card metric",
                    div { class: "metric-copy", span { class: "metric-label", "Affected IDs / Upstreams" } span { class: "metric-value", "{affected_value}" } span { class: "metric-note", "distinct IDs involved in HTTP errors" } }
                    div { class: "metric-icon tone-red", Icon { name: "routes" } }
                }
                div { class: "card metric",
                    div { class: "metric-copy", span { class: "metric-label", "Policy Controls" } span { class: "metric-value", "{policy_value}" } span { class: "metric-note", "enabled built-ins + custom rules" } }
                ...
                div { class: "card card-pad stack-lg",
                    ...
                    if filters_loading {
                        ...
                        div { class: "field",
                            ...
                            div { class: "row gap-2",
                                input { class: "input", value: "{new_rule}", placeholder: "Add a custom protection rule", oninput: move |event| new_rule.set(event.value()) }
                                button {
                                    class: "button button-secondary",
                                    onclick: move |_| {
                ...
                div { class: "card card-pad stack-lg",
                    div { class: "product-section-head",
                        div {
                            ...
                            p { "Circuit-breaker telemetry is shown as available only when the router actually returns its internal health payload." }
                        }
                    ...
                    if breaker_loading {
                        ...
                        div { class: "readiness-strip blocked",
                            ...
                            span { class: "muted", "Status could not be verified. Emergency stop remains available because a failed status check should not block an incident response." }
                        }
                    }
                    details {
                    ...
                    div { class: "product-note", "The current server returns circuit-breaker health as an untyped structured payload. BurnCloud keeps that raw payload on demand instead of inventing unsupported per-circuit labels." }
                }
            }
            ...
            div { class: "card table-card",
                ...
                if events_loading {
                    div { class: "card-pad", "Loading HTTP error signals…" }
                    ...
                    if page.data.is_empty() {
                        ...
                        div { class: "table-wrap",
                            table { class: "data-table",
                                thead { tr { th { "Time" } th { "Signal" } th { "Account / Source ID" } th { "Upstream / Path" } th { "HTTP" } } }
                                tbody {
            ...
            div { class: "card card-pad stack-lg danger-zone",
                ...
                p { class: "small muted", "Trip All Circuits is a real operational command. It can stop upstream routing across the environment. This action is deliberately separated from normal protection-policy editing." }
                div { class: "field",
