...
pub fn Customers() -> Element {
    ...
    let customer_count_text = if directory_ready { customer_count.to_string() } else { "—".to_string() };
    let active_count_text = if directory_ready { active_count.to_string() } else { "—".to_string() };
    let cny_total_text = if directory_ready { format_total_nano(cny_total, "CNY") } else { "—".to_string() };
    let usd_total_text = if directory_ready { format_total_nano(usd_total, "USD") } else { "—".to_string() };
    let active_note = if directory_ready {
        format!("{disabled_count} disabled")
    } else {
        "customer directory unavailable".to_string()
    };
    ...
    rsx! {
        div { class: "page",
            ...
            div { class: "metrics",
                ...
                div { class: "card metric",
                    div { class: "metric-copy", span { class: "metric-label", "Active" } span { class: "metric-value", "{active_count_text}" } span { class: "metric-note", "{active_note}" } }
                ...
                div { class: "card table-card",
                    div { class: "customer-toolbar",
                        div { class: "search-field customer-search",
                            Icon { name: "search" }
                            input { class: "input", placeholder: "Search customer, email or ID…", value: "{query}", oninput: move |event| query.set(event.value()) }
                        }
                    ...
                    if filtered.is_empty() {
                        ...
                        div { class: "table-wrap",
                            table { class: "data-table",
                                thead { tr { th { "Customer" } th { "Status" } th { "Wallet Balance" } th { "Preferred Currency" } th { class: "right", "Action" } } }
                                tbody {
                                    for user in filtered {
                                        {
                                            let email_text = user.email.clone().filter(|value| !value.trim().is_empty()).unwrap_or_else(|| "No email".to_string());
                                            ...
                                            let preferred = user.preferred_currency.clone().filter(|value| matches!(value.as_str(), "USD" | "CNY")).unwrap_or_else(|| "Not set".to_string());
                                            let user_for_topup = user.clone();
                                            rsx! {
                                                tr { key: "{user.id}",
                                                    td { div { class: "two-line", strong { class: "table-primary", "{user.username}" } small { class: "muted", "{email_text} • {user.id}" } } }
                                                    td { span { class: if user.status == 1 { "badge badge-success" } else { "badge badge-neutral" }, "{status_text}" } }
                                                    td { div { class: "two-line", strong { class: "tabular", "{cny_text}" } small { class: "tabular muted", "{usd_text}" } } }
                                                    td { "{preferred}" }
                                                    td { class: "right",
                                                        button {
                                                            ...
                                                            onclick: move |_| {
                                                                if user_for_topup.status != 1 { return; }
                                                                amount.set(String::new());
                                                                currency.set(user_for_topup.preferred_currency.clone().filter(|value| matches!(value.as_str(), "USD" | "CNY")).unwrap_or_else(|| "USD".to_string()));
                                                                notice.set(String::new());
                                                                error.set(String::new());
                                        ...
                                        match UserService::create(&username_value, &password_value, email_arg).await {
                                            Ok(created) => {
                                                if is_staff_roles(&created.roles) {
                                                    error.set(format!("Account {username_value} was created, but the server returned an admin role. Treat this as a privileged-account anomaly and review Team immediately."));
                                                } else {
                                                    notice.set(format!("Customer {username_value} created Active with the server's $10.00 USD signup credit."));
                                                }
            ...
            if let Some(user) = topup_user() {
                {
                    ...
                    let funding_text = if amount_nano > 0 { format_money_nano(amount_nano, &selected_currency) } else { "—".to_string() };
                    let current_balance_text = format_money_nano(current_balance_nano, &selected_currency);
                    let cny_balance_text = format_money_nano(user.balance_cny, "CNY");
                    let usd_balance_text = format_money_nano(user.balance_usd, "USD");
                    let balance_summary = format!("Current balances: {cny_balance_text} • {usd_balance_text}");
                    let after_text = if amount_nano > 0 {
                        current_balance_nano.checked_add(amount_nano).map(|balance| format_money_nano(balance, &selected_currency)).unwrap_or_else(|| "Amount too large".to_string())
                    } else {
                        current_balance_text.clone()
                    };
                    let confirm_label = if amount_nano > 0 { format!("Add {funding_text}") } else { "Add Funds".to_string() };
                    ...
                    rsx! {
                        ...
                        aside { class: "drawer",
                            ...
                            div { class: "drawer-body stack-lg",
                                if !account_active {
                                    div { class: "readiness-strip blocked", span { class: "readiness-dot" } strong { "This customer is disabled" } span { class: "muted", "Funding is blocked until account status is intentionally restored outside this page." } }
                                }
                                div { class: "form-section",
                                    div { class: "form-section-head", strong { "{user.username}" } small { "{balance_summary}" } }
                                    div { class: "grid-2",
                                        div { class: "field",
                                            label { "Currency" }
                                        ...
                                        div { class: "field",
                                            label { "Amount" }
                                            input { class: "input", r#type: "number", min: "0.01", step: "0.01", value: "{amount}", placeholder: "0.00", disabled: busy() || !account_active, oninput: move |event| amount.set(event.value()) }
                                        }
                                    }
                                    div { class: "row gap-2",
                                ...
                                div { class: "form-section",
                                    ...
                                    div { class: "receipt-row", label { "Current balance" } strong { class: "tabular", "{current_balance_text}" } }
                                    ...
                                    if let Err(message) = parsed_amount.clone() {
                                        if !amount().trim().is_empty() { div { class: "terminal auth-status auth-status-error", "{message}" } }
                                    }
                                }
                                if !error().is_empty() { div { class: "terminal auth-status auth-status-error", "{error}" } }
                                div { class: "row customer-form-actions",
                                            ...
                                            spawn(async move {
                                                match UserService::topup(&user_id, amount_nano, &selected_currency).await {
                                                    Ok(server_balance) => {
                                                        ...
                                                        notice.set(format!("Added {display_amount} to {user_name}. Server-confirmed {selected_currency} balance: {balance_text}."));
                                                        topup_user.set(None);
                                                        users_resource.restart();
                                                    }
