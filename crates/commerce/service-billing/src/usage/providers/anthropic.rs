...
mod tests {
    ...
    fn test_parse_response_basic() {
        ...
        let u = parser
            .parse_response(&resp)
            .unwrap_or_else(|e| panic!("parse_response failed: {e}"));
    ...
    fn test_parse_streaming_message_start() {
        ...
        let u = parser
            ...
            .unwrap_or_else(|e| panic!("parse_streaming_chunk failed: {e}"))
            .unwrap_or_else(|| panic!("expected Some(usage) for message_start"));
        assert_eq!(u.input_tokens, 50);
    ...
    fn test_parse_streaming_message_delta() {
        ...
        let u = parser
            .parse_streaming_chunk(line)
            .unwrap_or_else(|e| panic!("parse_streaming_chunk failed: {e}"))
            .unwrap_or_else(|| panic!("expected Some(usage) for message_delta"));
        assert_eq!(u.output_tokens, 75);
    }
    ...
    fn test_parse_streaming_cache_in_message_start() {
        ...
        let u = parser
            .parse_streaming_chunk(line)
            .unwrap_or_else(|e| panic!("parse_streaming_chunk failed: {e}"))
            .unwrap_or_else(|| panic!("expected Some(usage) for message_start with cache"));
        assert_eq!(u.input_tokens, 100);
