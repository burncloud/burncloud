...
use axum::{routing::post, Router, extract::State, body::Body, response::Response};
...
async fn test_group_routing_logic() -> anyhow::Result<()> {
    ...
    let upstream_app = Router::new()
        .route("/upstream-a/v1/group-chat/chat/completions", post(|State(s): State<Arc<MockUpstreamState>>| async move {
            let mut c = s.count_a.lock().unwrap();
            *c += 1;
            Response::new(Body::from(r#"{"id":"mock-a"}"#))
        }))
        .route("/upstream-b/v1/group-chat/chat/completions", post(|State(s): State<Arc<MockUpstreamState>>| async move {
    ...
    client.post(format!("{}/channels", api_base)).json(&chan_a).send().await?.error_for_status()?;
    ...
    client.post(format!("{}/channels", api_base)).json(&chan_b).send().await?.error_for_status()?;
    ...
    client.post(format!("{}/groups", api_base)).json(&group).send().await?.error_for_status()?;
    ...
    client.put(format!("{}/groups/{}/members", api_base, group_id)).json(&members).send().await?.error_for_status()?;

    // Force Config Reload (Internal API)
    client.post(format!("http://127.0.0.1:{}/console/internal/reload", gateway_port)).send().await?.error_for_status()?;
    sleep(Duration::from_millis(500)).await;

    // 5. Create a User Token to allow access
    let token_req = serde_json::json!({ "user_id": "test-user" });
    let token_resp = client.post(format!("{}/tokens", api_base)).json(&token_req).send().await?.json::<serde_json::Value>().await?;
    let user_token = token_resp["token"].as_str().unwrap();

    // 6. Send Requests to Group Path
    let gateway_url = format!("http://127.0.0.1:{}/v1/group-chat/chat/completions", gateway_port);
