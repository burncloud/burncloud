...
async fn test_user_flow() {
    ...
    let username = format!("user_{}", Uuid::new_v4().to_string().replace("-", "")[..8].to_string());
    ...
    let reg_res = client.post("/console/api/user/register", &reg_body).await.expect("Register failed");
    assert_eq!(reg_res["success"], true, "Register response: {:?}", reg_res);
    let user_id = reg_res["data"]["id"].as_str().expect("No ID returned").to_string();
    ...
    let login_res = client.post("/console/api/user/login", &login_body).await.expect("Login failed");
    assert_eq!(login_res["success"], true, "Login response: {:?}", login_res);
    ...
    let invalid_res = client.post("/console/api/user/login", &invalid_login_body).await.expect("Login request failed");
    assert_eq!(invalid_res["success"], false);
    
    // 4. List Users (As Admin) - Assuming no auth needed for MVP or root token
    let list_res = client.get("/console/api/list_users").await.expect("List users failed");
    ...
    let topup_res = client.post("/console/api/user/topup", &topup_body).await.expect("Topup failed");
