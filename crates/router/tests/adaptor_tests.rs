...
async fn test_zai_adaptor() -> anyhow::Result<()> {
    ...
    assert!(
        resp_json["usage"]["prompt_tokens"].is_i64()
            || resp_json["usage"]["prompt_tokens"].is_u64()
    );
    assert!(
        resp_json["usage"]["completion_tokens"].is_i64()
            || resp_json["usage"]["completion_tokens"].is_u64()
    );
...
async fn test_zai_streaming_adaptor() -> anyhow::Result<()> {
    ...
    assert_eq!(
        status, 200,
        "Expected 200 status for streaming, got {}",
        status
    );
    ...
    println!(
        "z.ai Streaming Response (first 500 chars): {}",
